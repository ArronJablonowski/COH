...
func testObservation(mode string, p providercontract.ProviderIdentity) LaunchObservation {
	return LaunchObservation{RuntimeVersion: RuntimeVersion, RuntimeDigest: RuntimeDigest, ProtocolDigest: ProtocolDigest, Model: p.ActualModel, ModelRevision: p.ModelRevision, Workspace: "/workspace", Transport: mode, Sandbox: "read-only", ApprovalPolicy: "untrusted", NetworkMode: "connected", ConfigDigest: p.ChatTemplateDigest, EnvironmentDigest: p.HardwareProfileDigest, CredentialMode: "invocation-scoped", ExperimentalSurface: map[bool]string{true: "native-tools-denied", false: "dynamicTools-only"}[mode == "exec-jsonl"], CodexHome: "/managed/codex-home", ConfigMode: "managed-isolated", RulesMode: "disabled", HooksMode: "disabled", MCPMode: map[bool]string{true: "disabled", false: "broker-only"}[mode == "exec-jsonl"], WebSearchMode: "disabled", MutationMode: "disabled", EnvironmentMode: "allowlist"}
}
