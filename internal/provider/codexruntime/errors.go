...
func (value *Error) Error() string {
	return fmt.Sprintf("Codex runtime adapter %s: %s", value.Code, value.Reason)
}
...
func Reason(err error) string {
	...
	return "codexruntime_transport_unavailable"
}
...
func Provenance(err error) string {
	var adapterError *Error
	if errors.As(err, &adapterError) {
		return adapterError.ProvenanceDigest
	}
	return ""
}
