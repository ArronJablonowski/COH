...
func NewLoopbackHTTPClient(timeout time.Duration) (*http.Client, error) {
	...
	transport.DialContext = func(ctx context.Context, network, address string) (net.Conn, error) {
		...
		if err != nil || (network != "tcp" && network != "tcp4") || host != "127.0.0.1" || port != "8000" {
...
func (adapter *Adapter) checkHealth(ctx context.Context) error {
	response, err := adapter.startRequest(ctx, http.MethodGet, HealthPath, nil, "")
	if err != nil {
		return err
	}
	...
	if err != nil {
		return newError(providercontract.Unavailable, "response_read_failed", true)
	}
	if len(body) != 0 {
		return newError(providercontract.Denied, "health_response_invalid", false)
	}
	return nil
}
...
func (adapter *Adapter) startRequest(ctx context.Context, method, path string, input any, accept string) (*http.Response, error) {
	...
	if request.URL.User != nil || request.Header.Get("Authorization") != "" || request.Header.Get("X-API-Key") != "" {
	...
	if response.TLS != nil || response.Request == nil || response.Request.Method != method || response.Request.URL.String() != adapter.config.Endpoint+path || response.Request.URL.User != nil || response.Request.Header.Get("Authorization") != "" || response.Request.Header.Get("X-API-Key") != "" {
		response.Body.Close()
	...
	if accept != "" {
		...
		if parseErr != nil || mediaType != accept {
			response.Body.Close()
...
func allowedOperation(method, path string) bool {
	return method == http.MethodGet && (path == HealthPath || path == VersionPath || path == ModelsPath || path == TokenizerInfoPath) || method == http.MethodPost && path == ChatPath
}
