...
func TestResponseSurfaceFailsClosed(t *testing.T) {
	tests := []struct {
		...
		{"fingerprint drift", func(value string) string { return strings.Replace(value, testDigest("8"), testDigest("7"), 1) }, providercontract.Denied},
		...
		{"log probabilities", func(value string) string {
			return strings.Replace(value, `"logprobs":null`, `"logprobs":{}`, 1)
		}, providercontract.Denied},
	}
...
func TestCancellationLimitsAndRecovery(t *testing.T) {
	...
	t.Run("canceled during chat", func(t *testing.T) {
		...
		if _, err := rig.adapter.Invoke(ctx, rig.request); Code(err) != providercontract.Canceled ||
			Reason(err) != "request_canceled" || len(rig.http.requests) != 5 {
			t.Fatalf("err=%v requests=%d", err, len(rig.http.requests))
		}
	})
...
func TestRuntimeIdentityProbesFailClosed(t *testing.T) {
	tests := []struct {
		name, path, fixture, old, replacement string
		requests                              int
	}{
		{"context drift", ModelsPath, "models.json", `"max_model_len": 32768`, `"max_model_len": 16384`, 4},
		{"model parent", ModelsPath, "models.json", `"parent": null`, `"parent": "lora"`, 3},
		{"template missing", TokenizerInfoPath, "tokenizer-info.json", `"chat_template": "`, `"chat_template": "", "old": "`, 4},
		{"version syntax", VersionPath, "version.json", `"0.11.0"`, `"floating-main"`, 2},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			rig := newTestRig(t)
			rig.http.responses[routeKey(http.MethodGet, test.path)] = routeResponse{status: http.StatusOK,
				body: []byte(strings.Replace(string(readFixture(t, test.fixture)), test.old, test.replacement, 1))}
			_, err := rig.adapter.Invoke(context.Background(), rig.request)
...
func TestHealthAndTokenizerProbesFailClosed(t *testing.T) {
	t.Run("health body", func(t *testing.T) {
		rig := newTestRig(t)
		rig.http.responses[routeKey(http.MethodGet, HealthPath)] = routeResponse{status: http.StatusOK, body: []byte(`{"status":"ok"}`)}
		_, err := rig.adapter.Invoke(context.Background(), rig.request)
		if Code(err) != providercontract.Denied || Reason(err) != "health_response_invalid" || len(rig.http.requests) != 1 {
			t.Fatalf("err=%v requests=%d", err, len(rig.http.requests))
		}
	})
	t.Run("tokenizer init kwargs drift", func(t *testing.T) {
		rig := newTestRig(t)
		body := strings.Replace(string(readFixture(t, "tokenizer-info.json")), `"clean_up_tokenization_spaces": false`, `"clean_up_tokenization_spaces": true`, 1)
		rig.http.responses[routeKey(http.MethodGet, TokenizerInfoPath)] = routeResponse{status: http.StatusOK, body: []byte(body)}
		_, err := rig.adapter.Invoke(context.Background(), rig.request)
		if Code(err) != providercontract.Denied || Reason(err) != "observed_identity_drift" || len(rig.http.requests) != 4 {
			t.Fatalf("err=%v requests=%d", err, len(rig.http.requests))
		}
	})
}
...
func TestConfigurationAndLoopbackTransportDenyRouteExpansion(t *testing.T) {
	rig := newTestRig(t)
	tests := []func(*Config){
		func(config *Config) { config.Endpoint = "http://localhost:8000" },
		func(config *Config) { config.Endpoint = "https://vllm.example" },
