...
func newTestRig(t *testing.T) testRig {
	...
	httpClient := &routeHTTPStub{responses: map[string]routeResponse{
		routeKey(http.MethodGet, HealthPath):     {status: http.StatusOK, body: readFixture(t, "health.json")},
		routeKey(http.MethodGet, PropertiesPath): {status: http.StatusOK, body: readFixture(t, "properties.json")},
	...
	adapter, err := New(Config{Endpoint: LlamaCPPEndpoint, Capability: capability,
...
func testProvider(t *testing.T) providercontract.ProviderIdentity {
	...
	infoDigest, err := validateModels(models, "qwen3-8b-coh", contextLimit)
	if err != nil {
		t.Fatal(err)
	}
	...
	return providercontract.ProviderIdentity{ProviderKind: "llama_cpp", AdapterVersion: AdapterVersion,
		EndpointIdentityDigest: EndpointIdentityDigest(LlamaCPPEndpoint), DataRoute: "local", RequestedModel: "qwen3-8b-coh",
		ActualModel: "qwen3-8b-coh", ModelRevision: testDigest("a"), RuntimeName: "llama.cpp",
		RuntimeVersion: runtimeVersion, RuntimeDigest: testDigest("8"),
		TokenizerName: "gguf", TokenizerVersion: runtimeVersion,
		TokenizerDigest: infoDigest, ChatTemplateDigest: digest(templateDigestDomain, []byte(properties.ChatTemplate)),
