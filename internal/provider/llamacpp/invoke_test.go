...
func TestInvokeUsesQualifiedChatCompletionsSurface(t *testing.T) {
	...
	if value.Outcome != "succeeded" || value.Provider.ModelRevision != testDigest("a") ||
		...
		value.Items[2].CallID != "llamacpp:0198e300-3000-7000-8000-000000000003:0" {
		t.Fatalf("response=%+v", value)
	}
	expected := []string{routeKey(http.MethodGet, HealthPath), routeKey(http.MethodGet, PropertiesPath),
		routeKey(http.MethodGet, ModelsPath), routeKey(http.MethodPost, ChatPath)}
	if len(rig.http.requests) != len(expected) {
		t.Fatalf("requests=%d", len(rig.http.requests))
	}
	for index, request := range rig.http.requests {
		if routeKey(request.Method, request.URL.Path) != expected[index] || request.URL.Host != "127.0.0.1:8080" ||
	...
	if chat.Model != "qwen3-8b-coh" || chat.Stream || chat.StreamOptions != nil || chat.CachePrompt ||
		!chat.ParseToolCalls || !chat.ParallelToolCalls || chat.ToolChoice != "auto" || chat.MaximumTokens != 1024 ||
...
func TestManagedRouteReceivesExactBinaryAndGGUFBinding(t *testing.T) {
	rig := newTestRig(t)
	...
	if observed.Endpoint != LlamaCPPEndpoint || observed.BuildInfo != "b7000-5d5cb4c3" ||
		observed.ExpectedRuntimeDigest != testDigest("8") || observed.ModelAlias != "qwen3-8b-coh" ||
		observed.ModelPath != "/models/Qwen3-8B-Q4_K_M.gguf" || observed.ExpectedGGUFDigest != testDigest("a") ||
		observed.ChatTemplateDigest != rig.request.Value().Provider.ChatTemplateDigest {
