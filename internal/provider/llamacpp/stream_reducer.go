...
func newStreamReducer(adapter *Adapter, ctx context.Context, validated providercontract.ValidatedRequest,
	...
	return &streamReducer{adapter: adapter, ctx: ctx, validatedRequest: validated, request: request, tools: tools,
		emit: emit, toolCalls: make(map[uint64]*toolAccumulator)}
}
...
func (reducer *streamReducer) consume(canonical []byte, chunk streamChunk) error {
	...
	if chunk.Usage != nil || chunk.Timings != nil || len(chunk.Choices) != 1 {
...
func (reducer *streamReducer) bindChunk(chunk streamChunk) error {
	runtimeVersion, runtimeErr := normalizeBuildInfo(chunk.SystemFingerprint)
	if !validText(chunk.ID, 128) || !strings.HasPrefix(chunk.ID, "chatcmpl-") ||
		...
		runtimeErr != nil || runtimeVersion != reducer.request.Provider.RuntimeVersion {
...
func (reducer *streamReducer) consumeDelta(delta streamDelta) error {
	if delta.Role != "" {
		if delta.Role != "assistant" || reducer.text.Len() != 0 || reducer.reasoning.Len() != 0 || len(reducer.toolCalls) != 0 ||
			delta.Content != nil && *delta.Content != "" || delta.ReasoningContent != nil || len(delta.ToolCalls) != 0 {
	...
	if delta.ReasoningContent != nil {
		if *delta.ReasoningContent == "" || reducer.reasoning.Len()+len(*delta.ReasoningContent) > maximumResponseBytes {
			return newError(providercontract.InvalidInput, "stream_reasoning_delta", false)
		}
		reducer.reasoning.WriteString(*delta.ReasoningContent)
		populated++
	}
...
func (reducer *streamReducer) consumeFinish(choice streamChoice) error {
	if reducer.finishReason != "" || choice.Delta.Role != "" || choice.Delta.Content != nil ||
		choice.Delta.ReasoningContent != nil || len(choice.Delta.ToolCalls) != 0 {
...
func (reducer *streamReducer) consumeUsage(chunk streamChunk) error {
	if reducer.finishReason == "" || chunk.Usage == nil || chunk.Timings == nil {
	...
	vendor := chatResponse{ID: chunk.ID, Object: "chat.completion", Created: chunk.Created, Model: chunk.Model,
		SystemFingerprint: chunk.SystemFingerprint, Choices: []chatChoice{{Index: 0, Message: message,
			FinishReason: reducer.finishReason}}, Usage: *chunk.Usage, Timings: *chunk.Timings}
	...
	reducer.pending, err = reducer.adapter.translateResponse(reducer.ctx, reducer.validatedRequest,
		reducer.request, reducer.tools, vendor, trace)
	if err != nil {
		return err
	}
...
func (reducer *streamReducer) completeMessage() (chatMessage, error) {
	...
	for ordinal, index := range indices {
		if ordinal != index {
			return chatMessage{}, newError(providercontract.Conflict, "stream_tool_index", false)
		}
		item := reducer.toolCalls[uint64(index)]
		if item.name.Len() == 0 || item.arguments.Len() == 0 {
			return chatMessage{}, newError(providercontract.InvalidInput, "stream_tool_incomplete", false)
		}
		value := uint64(index)
	...
	message := chatMessage{Role: "assistant", Content: reducer.text.String(),
		ReasoningContent: reducer.reasoning.String(), ToolCalls: calls}
	if message.Content == "" && message.ReasoningContent == "" && len(message.ToolCalls) == 0 {
		return chatMessage{}, newError(providercontract.InvalidInput, "vendor_response_empty", false)
	}
	return message, nil
}
