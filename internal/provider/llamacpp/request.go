...
const reasoningDigestDomain = "COH-LLAMACPP-REASONING-V1\x00"
...
func (adapter *Adapter) translateRequest(ctx context.Context, request providercontract.InferenceRequest) (requestTranslation, error) {
	...
	translation.wire = chatRequest{Model: request.Provider.RequestedModel, MaximumTokens: request.MaximumOutputTokens,
		...
		Stream: false, CachePrompt: false, ParseToolCalls: len(request.Tools) > 0,
	...
	if request.OutputConstraint.Kind == "json_schema" {
		...
		translation.wire.ResponseFormat = &responseFormat{Type: "json_schema", Schema: schema}
	}
...
func (adapter *Adapter) translateMessage(ctx context.Context, message providercontract.Message,
	...
	for _, item := range message.Items {
		...
		case "reasoning_ref":
			if wire.ReasoningContent != "" || message.Role != "assistant" {
			...
			if err := decodeExact(canonical, &envelope); err != nil || !validText(envelope.ReasoningContent, 1<<20) {
				return chatMessage{}, newError(providercontract.Denied, "reasoning_item_invalid", false)
			}
			wire.ReasoningContent = envelope.ReasoningContent
		case "tool_call":
	...
	if strings.TrimSpace(wire.Content) == "" && wire.ReasoningContent == "" && len(wire.ToolCalls) == 0 {
