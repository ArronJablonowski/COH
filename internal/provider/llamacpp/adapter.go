...
func validateConfig(config Config) error {
	if config.Endpoint != LlamaCPPEndpoint || config.Capability.Digest() == "" || config.Qualifications == nil ||
		config.Schemas == nil || config.Reasoning == nil || config.Tokens == nil || config.Route == nil ||
		config.HTTP == nil || config.Clock == nil {
	...
	if provider.ProviderKind != "llama_cpp" || provider.AdapterVersion != AdapterVersion ||
		provider.EndpointIdentityDigest != EndpointIdentityDigest(config.Endpoint) || provider.DataRoute != "local" ||
