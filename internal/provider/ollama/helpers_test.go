...
func newTestRig(t *testing.T) testRig {
	...
	httpClient := &routeHTTPStub{responses: map[string]routeResponse{
		...
		routeKey(http.MethodGet, TagsPath):    {status: http.StatusOK, body: readFixture(t, "tags.json")},
		routeKey(http.MethodPost, ShowPath):   {status: http.StatusOK, body: readFixture(t, "show.json")},
...
func testProvider(t *testing.T) providercontract.ProviderIdentity {
	...
	record := modelRecord{Name: "qwen3:8b", Model: "qwen3:8b", ModifiedAt: show.ModifiedAt, Size: 5220000000,
		Digest: strings.Repeat("a", 64), Details: show.Details}
	contextLimit, infoDigest, err := validateShow(show, record)
	if err != nil {
		t.Fatal(err)
	}
	return providercontract.ProviderIdentity{ProviderKind: "ollama", AdapterVersion: AdapterVersion,
		EndpointIdentityDigest: EndpointIdentityDigest(OllamaEndpoint), DataRoute: "local", RequestedModel: "qwen3:8b",
		ActualModel: "qwen3:8b", ModelRevision: testDigest("a"), RuntimeName: "ollama", RuntimeVersion: "0.12.6",
		RuntimeDigest: digest(runtimeDigestDomain, []byte("0.12.6")), TokenizerName: "qwen3", TokenizerVersion: "0.12.6",
		TokenizerDigest: infoDigest, ChatTemplateDigest: digest(templateDigestDomain, []byte(show.Template)),
