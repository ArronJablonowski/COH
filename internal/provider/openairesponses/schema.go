...
func (adapter *Adapter) resolveSchema(ctx context.Context, digest string) (json.RawMessage, error) {
	document, err := adapter.config.Schemas.Resolve(ctx, digest)
	if err != nil {
		return nil, newError(providercontract.Unavailable, "schema_resolution_failed", true)
	}
	if document.Digest != digest || len(document.JSON) == 0 || len(document.JSON) > providercontract.MaximumInputBytes {
