...
func (sink *DurableAuditSink) AppendOIDCEvent(ctx context.Context, source Event) error {
	subjectID := source.SessionID
	if subjectID == "" {
		subjectID = source.StateID
	}
	evidence := []string{}
	for _, digest := range []string{source.ProfileDecisionDigest, source.IssuerDigest, source.SubjectDigest, source.KeyIDDigest} {
		if digest != "" {
	...
	event := tamperaudit.Event{SchemaVersion: tamperaudit.EventSchemaVersion, ContractVersion: tamperaudit.ContractVersion,
		...
		SourceSchema: SchemaVersion, Operation: source.Event, Outcome: source.Outcome,
		ReasonCode: source.ReasonCode, SubjectID: subjectID, EvidenceDigests: evidence,
		OccurredAt: oidcAuditTime(source.OccurredAt)}
...
func oidcAuditTime(value time.Time) string {
