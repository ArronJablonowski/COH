...
func TestLocalAuthenticationHasNoNetworkProcessOrLoggingSurface(t *testing.T) {
	...
	for _, entry := range entries {
		...
		for _, imported := range file.Imports {
			...
			if denied[path] {
				t.Errorf("%s imports forbidden authentication-boundary package %q", entry.Name(), path)
			}
		}
	}
}
