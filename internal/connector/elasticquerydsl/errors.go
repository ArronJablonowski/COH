...
func contextError(ctx context.Context) error {
	...
	if err := ctx.Err(); err != nil {
		if errors.Is(err, context.DeadlineExceeded) {
			return queryconnector.NewError(queryconnector.Timeout, "querydsl_validation_timeout", err)
		}
		return queryconnector.NewError(queryconnector.Canceled, "querydsl_validation_canceled", err)
	}
	return nil
}
