...
func (service *ExportService) appendAndVerifyAudit(ctx context.Context, scope domain.CaseRef,
	...
	if err := service.auditor.AppendLifecycleEvent(auditCtx, event); err != nil {
		return newError(Unavailable, "export_audit_append_unavailable", true, err)
	}
	if err := service.auditor.VerifyLifecycleEvent(auditCtx, scope, event.EventID, expected); err != nil {
		return newError(Unavailable, "export_audit_verification_unavailable", true, err)
	}
	return nil
}

func (service *ExportService) auditDenial(ctx context.Context, command Command, failure error) error {
	if ctx == nil || command.Operation != Export || validateCommandShape(command) != nil {
		return nil
	}
	...
	digests := []string{intent, *command.ArtifactSetDigest, *command.PurposeDigest, *command.DestinationDigest,
		*command.ApprovalDigest, command.PolicyDigest, command.ExpectedCustodyHead.ChainHash}
	...
	event := tamperaudit.Event{SchemaVersion: tamperaudit.EventSchemaVersion, ContractVersion: tamperaudit.ContractVersion,
		...
		Operation: "evidence.export.denied", Outcome: "denied", ReasonCode: reason,
	...
	if tamperaudit.ValidateEvent(event) != nil {
		return newError(Unavailable, "export_denial_audit_invalid", false, nil)
	}
	canonical, err := tamperaudit.CanonicalEvent(event)
	if err != nil {
		return newError(Unavailable, "export_denial_audit_invalid", false, err)
	}
	expected := digest("COH-EVIDENCE-LIFECYCLE-AUDIT-EVENT-V1\x00", canonical)
	return service.appendAndVerifyAudit(ctx, command.Case, event, expected)
}
