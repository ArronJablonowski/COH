...
func (service *ImportService) appendAndVerifyImportAudit(ctx context.Context, scope domain.CaseRef,
	...
	if err := service.auditor.AppendLifecycleEvent(auditCtx, event); err != nil {
		return newError(Unavailable, "import_audit_append_unavailable", true, err)
	}
	if err := service.auditor.VerifyLifecycleEvent(auditCtx, scope, event.EventID, expected); err != nil {
		return newError(Unavailable, "import_audit_verification_unavailable", true, err)
	}
	return nil
}

func (service *ImportService) auditImportDenial(ctx context.Context, command Command, failure error) error {
	if ctx == nil || command.Operation != Import || validateCommandShape(command) != nil {
		return nil
	}
	...
	digests := []string{intent, *command.PackageDigest, *command.SourceDigest, command.PolicyDigest,
	...
	event := tamperaudit.Event{SchemaVersion: tamperaudit.EventSchemaVersion, ContractVersion: tamperaudit.ContractVersion,
		...
		Operation: "evidence.import.denied", Outcome: "denied", ReasonCode: reason,
	...
	if tamperaudit.ValidateEvent(event) != nil {
		return newError(Unavailable, "import_denial_audit_invalid", false, nil)
	}
	canonical, err := tamperaudit.CanonicalEvent(event)
	if err != nil {
		return newError(Unavailable, "import_denial_audit_invalid", false, err)
	}
	expected := digest("COH-EVIDENCE-LIFECYCLE-AUDIT-EVENT-V1\x00", canonical)
	return service.appendAndVerifyImportAudit(ctx, command.Case, event, expected)
}
