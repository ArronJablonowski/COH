...
for go_path in "${GOCACHE}" "${GOMODCACHE}" "${GOPATH}" "${GOTMPDIR}" "${TMPDIR}" "${XDG_CONFIG_HOME}" "${XDG_CACHE_HOME}"; do
...
[[ "$("${go_cmd}" telemetry)" == "off" ]] || { echo "Go telemetry must remain off" >&2; exit 1; }
